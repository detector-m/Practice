...
* 调用时机
	...
	* load：load方法，在类或者类目所在的文件被引用（即运行时加载该类或类目）时被调用，在main函数调用前。
	* initialize: initialize方法，只有在第一次向类/子类发送消息（第一次调用）时才会调用，而且只会调用一次。在main函数之后。
...
* 调用规则：
	...
	* initialize:
		...
		2. 类目的initialize会覆盖主类的initialize方法。存在分类的情况下，调用最后编译的分类类目的initialize方法。且都是先调用父类，再到子类。
